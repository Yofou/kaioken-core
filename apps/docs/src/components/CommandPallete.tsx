...
const CommandPaletteItem : Kaioken.FC<{ name: string, href: string }> = (props) => {
...
export const CommandPalette: Kaioken.FC<{ container: Kaioken.Ref<HTMLElement | null> }> = (props) => {
  ...
  return isClient && <Portal container={document.body}>
    <div className="w-full h-screen fixed top-0 left-0 z-10 flex justify-center p-12 items-start bg-black/50" ariaLabel={"Escape to close"} ariaLive={"polite"}>
      <DemoContainer ref={props.container} className="w-[750px] p-4">
        ...
        />
        <div className="w-full p-2 flex flex-col rounded-lg  mt-4 gap-4 h-[300px] overflow-y-auto">
